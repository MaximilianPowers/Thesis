...
def animate_run(LvS, size=(5, 5), fps=15, bitrate=1800, filename='run',
                ticks='off', dpi=100, cmap=cmap1, filepath=None):
    ...
    ani.save(filepath + filename + '.gif', dpi=dpi, writer=writer)


def fast_animate_run(LvS, resize=True, size=(200, 200), fastest=True, filename='run',
                     ticks='off', cmap=cmap1, filepath=None):
    ...
    """
    if fastest:
        gif = [Image.fromarray(np.uint8(L)).convert('RGB') for L in tqdm(LvS)]
    else:
        gif = [Image.fromarray(np.uint8(cmap(L)*255)) for L in tqdm(LvS)]
    
    if resize:
        gif = [img.resize(size) for img in gif]
    print(len(gif))
    gif[0].save(filepath + filename + '.gif', save_all=True, optimize=False, append_images=gif[1:], loop=0)


if __name__ == '__main__':

    start_time = time.time()
    L = initialize_lattice_random(512, 512)
    summary, frames = cooling(L, .5*np.ones(20), 20)
    fast_animate_run(frames, fastest=False, size=(500, 500), filepath="Figures/Ising1/", filename="run_temp_const_1")
