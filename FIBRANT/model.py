...
class BioMLP(nn.Module):
    ...
    def swap(self, i, j, k):
        ...
        if ((i, j) in self.removed_nodes) or ((i, k) in self.removed_nodes):
            return
    ...
    def compute_fisher_information(self, validation_loader):
        # Initialize the FIM
        total_fim = None
        num_samples = 0
        ...
        for inputs, _ in validation_loader:
            ...
            self.kfac.compute_fisher(inputs, outputs)
        ...
        # Average the FIM over the entire validation set
        for key in total_fim.keys():
            total_fim[key] = total_fim[key] / num_samples
    ...
    def rank_nodes_by_importance(self):
        ...
        for layer_name, (FIM_in, FIM_out) in self.kfac.fisher.items():
            # The importance of a node is the sum of the FIM values for its parameters
            importance_in = FIM_in.sum(dim=1)
            importance_out = FIM_out.sum(dim=0)

            # Store the importance in the dictionary
            node_importance[layer_name + "_in"] = importance_in
            node_importance[layer_name + "_out"] = importance_out
