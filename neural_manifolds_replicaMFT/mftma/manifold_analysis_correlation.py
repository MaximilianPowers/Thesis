...
def manifold_analysis_corr(XtotT, kappa, n_t, t_vecs=None, n_reps=10):
    ...
    XtotT = [x for x in XtotT if len(np.array(x).shape) != 0]
    ...
    # Centers is of shape (N, m) for m manifolds
...
def each_manifold_analysis_D1(sD1, kappa, n_t, eps=1e-8, t_vec=None):
    ...
    lamb = np.asarray([max_ts[i]/s_sum[i] if s_sum[i]
                      > 0 else 0 for i in range(n_t)])
...
def fun_FA(centers, maxK, max_iter, n_repeats, s_all=None, verbose=False, conjugate_gradient=True):
    ...
    V1 = None
    print(
        f"Running Conjugate Gradient Descent on atmost {maxK} Dimensions {n_repeats} times.")
    for i in range(1, maxK + 1):
        ...
        for j in range(1, n_repeats + 1):
            ...
            V1tmp, output = CGmanopt(V0, partial(
                square_corrcoeff_full_cost, grad=False), X, **opts)
            ...
            assert np.linalg.norm(
                np.matmul(V1tmp.T, V1tmp) - np.identity(i), ord='fro') < 1e-10
...
def square_corrcoeff_full_cost(V, X, grad=True):
    ...
    C = np.matmul(X, X.T)
    c = np.matmul(X, V)
    c0 = np.diagonal(C).reshape(P, 1) - \
        np.sum(np.square(c), axis=1, keepdims=True)
