...
def analyze(model, dataset, sampled_classes=50, examples_per_class=50, kappa=0, n_t=300, n_reps=1,
    ...
    manifold_data = make_manifold_data(
        dataset, sampled_classes, examples_per_class, seed=seed)
    # Move the model and data to the device
    model = model.to(device)
    manifold_data = [d.to(device) for d in manifold_data]
    # Extract the activations
    activations = extractor(model, manifold_data,
                            layer_nums=layer_nums, layer_types=layer_types)
    ...
    for k, X, in activations.items():
        ...
        if analyze:
            ...
            a, r, d, r0, K = manifold_analysis_corr(
