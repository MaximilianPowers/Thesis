...
class BioMLP(nn.Module):
    def __init__(self, in_dim=2, out_dim=2, w=2, depth=2, shp=None, token_embedding=False, embedding_size=None, dist='L1', ystar=0.2, topk=6, sample_size=50):
    ...
    def compute_fim(self):
        self.fim = []
        for i in range(self.depth):
            input = self.inputs[i].detach()
            output = self.outputs[i].detach()
            grad_output = torch.ones_like(output)
            grad_input = torch.autograd.grad(
                output, input, grad_output, create_graph=True)[0]
            self.fim.append(torch.einsum(
                'bi,bj->bij', grad_input, grad_input).mean(0))
    ...
    def get_cc(self, weight_factor=1.0, bias_penalize=True, no_penalize_last=False):
        ...
        for i in range(num_linear):
            ...
            if self.dist == 'L1':
                distance = torch.abs(biolinear.out_coordinates.unsqueeze(
                    dim=1) - biolinear.in_coordinates.unsqueeze(dim=0))
            elif self.dist == 'Fisher':
                # Implement distance function for Fisher Information Matrix
            cc += torch.sum(torch.abs(biolinear.linear.weight)
                            * (weight_factor*distance+self.l0))
    ...
    def compute_fisher_information(self, inputs, targets):
        # Get all the parameters of the model
        params = list(self.parameters())

        # Flatten the parameters into a single vector
        params_flat = torch.cat([param.view(-1) for param in params])

        # Initialize the FIM to zero
        fim = torch.zeros(params_flat.size(0), params_flat.size(0))
        ...
        for input, target in zip(inputs, targets):
            ...
            # Compute the log probabilities of the output
            log_probs = F.log_softmax(output, dim=1)

            # Compute the derivative of the log probabilities with respect to the parameters
            grads = torch.autograd.grad(
                log_probs[target], params_flat, create_graph=True)

            # Compute the outer product of the gradients and add it to the FIM
            fim += torch.ger(grads[0], grads[0])

        # Divide by the number of input-target pairs to get the expectation
        fim /= inputs.size(0)

        return fim

    def update_fim_dict(self, model, X, y):
        fim_dict = {}

        indices = np.arange(len(X))
        np.random.shuffle(indices)
        sample = indices[:self.sample_size]
        X = X[sample]
        y = y[sample]

        # For each layer in the network...
        for i in range(len(model.linears) - 1):
            # Compute the FIM for the current layer
            fim = model.compute_fisher_information(X, y, model.linears[i])

            # Store the FIM in the dictionary
            fim_dict[(i, i+1)] = fim
            ...
            for i in range(N):
                if j == 0:
                    for fold in range(in_fold):
                        circle = Ellipse(
                            (1/(2*N)+i/N, 0.1*j+0.02*fold-0.01), s, s/10*((len(shp)-1)+0.4), color='black')
                        ax.add_patch(circle)
                else:
                    for fold in range(in_fold):
                        circle = Ellipse((1/(2*N)+i/N, 0.1*j), s,
                                         s/10*((len(shp)-1)+0.4), color='black')
                        ax.add_patch(circle)
                ...
                if ii == 0:
                    for fold in range(in_fold):
                        for j in range(fold_num):
                            plt.plot([1/(2*p_shp[0])+i/p_shp[0], 1/(2*fold_num)+j/fold_num], [0.1*(ii+1), 0.1*ii+0.02 *
                                     fold-0.01], lw=1*np.abs(p[i, j].detach().numpy()), color="blue" if p[i, j] > 0 else "red")
                else:
                    for j in range(fold_num):
                        plt.plot([1/(2*p_shp[0])+i/p_shp[0], 1/(2*fold_num)+j/fold_num], [0.1*(ii+1), 0.1*ii],
                                 lw=0.5*np.abs(p[i, j].detach().numpy()), color="blue" if p[i, j] > 0 else "red")
    ...
    def thresholding(self, threshold, checkpoint=True):
        num = 0
        if checkpoint:
            self.original_params = [param.clone()
