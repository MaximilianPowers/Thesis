...
from riemannian_geometry.computations.ricci import OllivierRicci, FormanRicci
...
def local_length(manifold, curve, t):
    ...
    if manifold.is_diagonal():
        # T x 1, c'(t) M(c(t)) c'(t)
        dist = np.sqrt(np.sum(M.transpose() * (dc ** 2), axis=0))
    else:
        dc = dc.T  # D x N -> N x D
        dc_rep = np.repeat(dc[:, :, np.newaxis], D,
                           axis=2)  # N x D -> N x D x D
...
def curve_length(manifold, curve, a=0, b=1, tol=1e-5, limit=50):
    ...
    if callable(curve):
        ...
        curve_length_eval = integrate.quad(lambda t: local_length(
            manifold, curve, t), a, b, epsabs=tol, limit=limit)  # , number of subintervals
    else:
...
def unit_speed_curve_euclidean(curve, t, N_points=1000):
    ...
    local_lengths_N = np.sqrt(np.sum(curve(T_without_boundary_points)[
                              1] ** 2, axis=0, keepdims=True)).T
    ...
    temp = np.concatenate(([[0]], lenght_cumsums.reshape(-1, 1),
                          [[total_length]]), axis=0).flatten() / total_length
    # Here we give the proportion of length to move on the curve, and we get the time
    new_time = CubicSpline(temp, T)
...
def curve_eval_gp(Ts, T, T_min_max, Veta, dm, m, DDC, w, gp_kernel):
    ...
    dmu_mu_Ts = vec(np.concatenate((dm(Ts), m(Ts))).T) + \
        np.kron(Ctest, Veta) @ w
...
def solver_comb(solver, manifold, c0, c1, solution):
    ...
    try:
        ...
        # If method failed
        def curve(t): return evaluate_failed_solution(c0, c1, t)
        logmap = (c1 - c0)  # D x 1
        solution = None
        failed = True
        curve_length_eval = curve_length(manifold, curve)
        # Scaling for normal coordinates
        logmap = curve_length_eval * \
            logmap.reshape(-1, 1) / np.linalg.norm(logmap)
...
def solver_bvp(solver, manifold, c0, c1, init_solution):
    ...
    def ode_fun(t, c_dc): return second2first_order(
        manifold, c_dc)  # D x T, implements c'' = f(c, c')

    def bc_fun(ya, yb): return boundary_conditions(
        ya, yb, c0, c1)  # 2D x 0, what returns?

    # Initialize the curve with straight line or with another given curve
    T = 30
    t_init = np.linspace(0, 1, T, dtype=np.float32)  # T x 0
    if init_solution is None:
        c_init = np.outer(c0, (1.0 - t_init.reshape(1, T))) + \
            np.outer(c1, t_init.reshape(1, T))  # D x T
    ...
    result = solve_bvp(ode_fun, bc_fun, t_init.flatten(),
                       c_dc_init, tol=solver.tol, max_nodes=solver.NMax)

    # Provide the output, if solver failed return the straight line as solution
    if result.success:
        def curve(t): return evaluate_solution(result, t)
    ...
    curve_length_eval = curve_length(manifold, curve)
    # Scaling for normal coordinates
    logmap = curve_length_eval * logmap.reshape(-1, 1) / np.linalg.norm(logmap)
...
def solver_fp(solver, manifold, c0, c1, solution=None):
    ...
    c1 = c1.reshape(-1, 1)
    D = c0.shape[0]
    C_given = np.concatenate(
        (c0.reshape(1, -1), c1.reshape(1, -1)), axis=0)  # 2 x D
    ...
    v = c1 - c0
    # Estimate in Bayesian style the amplitude
    Veta = ((v.T @ solver.Sdata @ v) * solver.Sdata)

    # Parametric functions of prior mean, dmean, ddmean
    def m(t): return c0.T + t * (c1 - c0).T  # T x D
    def dm(t): return np.ones((t.shape[0], 1)) * (c1 - c0).T  # T x D
    def ddm(t): return np.zeros((t.shape[0], 1)) * (c1 - c0).T  # T x D
    # 2N x D, keep a vector for speed of the m, dm on T
    dm_m_T = vec(np.concatenate((dm(T), m(T)), axis=0).T)
    ...
    if solution is None:
        ...
        # The points from the training data used in the graph
        C_near_curve = solution['points']
        # The variance (noise) of the extra points we consider for the curve
        var_extra_points = solution['noise']

        N_near_curve = C_near_curve.shape[0]
        T_given_points = solution['time_stamps'].reshape(-1, 1)

        # Add the points on the given vector
        C_given = np.concatenate((C_given, C_near_curve), axis=0)
        ...
        for n in range(N_near_curve):
            # The noise of the new points
            R = block_diag(R, var_extra_points * I_D)
        ...
        for n in range(N_near_curve):
            # The noise of the new points
            R_temp = block_diag(R_temp, var_extra_points * I_D)
        ...
        DDC = kronCtrainVeta_temp @ (Btrain_R_inv_temp @
                                     vec((y_obs_temp - y_hat_temp).T))
    ...
    def y_obs(ddc): return np.concatenate((ddc, C_given), axis=0)  # N+2 x D

    # Define the posterior mean for the knots t_n, parametrized by the ddc_n
    Btrain_R_inv = np.linalg.inv(
        np.kron(Btrain, Veta) + R)  # Precompute for speed
    kronCtrainVeta = np.kron(Ctrain, Veta)

    def dmu_mu_post(ddc): return dm_m_T + \
        kronCtrainVeta @ (Btrain_R_inv @ vec((y_obs(ddc) - y_hat).T))
    ...
    while True:
        ...
        dmu_mu_post_curr_temp = dmu_mu_post_curr.reshape(
            D, 2 * N, order='F')  # D x N
        dmu_post_curr = dmu_mu_post_curr_temp[:, :N]  # D x N
        mu_post_curr = dmu_mu_post_curr_temp[:, N:]  # D x N

        DDC_curr = geodesic_system(
            manifold, mu_post_curr, dmu_post_curr).T  # N x D
        cost_curr = (DDC - DDC_curr) ** 2
        # Check point-wise if lower than tol
        condition_1 = (cost_curr < tol).all()
        ...
        for i in range(3):
            ...
            dmu_mu_post_curr_temp = dmu_mu_post(
                DDC_temp).reshape(D, 2 * N, order='F')
            dmu_post_curr_temp = dmu_mu_post_curr_temp[:, :N]  # D x N
            mu_post_curr_temp = dmu_mu_post_curr_temp[:, N:]  # D x N

            cost_temp = (DDC_temp - geodesic_system(manifold,
                         mu_post_curr_temp, dmu_post_curr_temp).T) ** 2  # N x D
    ...
    if convergence_cond == 2:
        ...
        w = (Btrain_R_inv @ vec((y_obs(DDC) - y_hat).T))
        def curve(t): return curve_eval_gp(
            t, T, T_given, Veta, dm, m, DDC, w, gp_kernel)
        logmap = curve(0)[1].flatten()  # (D,)
        failed = False
        # Use a return variable for debugging
        solution = {'ddc': DDC, 'total_iter': iter,
                    'cost': cost_curr, 'curve': curve, 'solver': 'fp'}
...
def solver_graph(solver, manifold, c0, c1, solution=None):
    ...
    # Find the closest kNN_num+1 points to c0
    _, c0_indices = solver.kNN_graph.kneighbors(c0.T)
    # Find the closest kNN_num+1 points to c1
    _, c1_indices = solver.kNN_graph.kneighbors(c1.T)
    ind_closest_to_c0 = np.nan  # The index in the training data closer to c0
    ind_closest_to_c1 = np.nan
    cost_to_c0 = 1e10
    cost_to_c1 = 1e10
    # We added one extra neighbor when we constructed the graph
    for n in range(solver.kNN_num - 1):
        ...
        # The kNN point near to c0
        x_c0 = solver.data[ind_c0, :].reshape(-1, 1)
        # The kNN point near to c1
        x_c1 = solver.data[ind_c1, :].reshape(-1, 1)

        # Construct temporary straight lines
        def temp_curve_c0(t): return evaluate_failed_solution(c0, x_c0, t)
        def temp_curve_c1(t): return evaluate_failed_solution(c1, x_c1, t)
    ...
    while True:
        # The previous point to reach the [goal == temp_ind]
        prev_ind = solver.predecessors[source_ind, temp_ind]
        if prev_ind == -9999:  # There is not any other point in the path
            ...
            pairwise_lengths.append(W[temp_ind, prev_ind])
            # Move the pointer to one point close to the source.
            temp_ind = prev_ind
    ...
    # The discrete path on the graph
    DiscreteCurve_data = solver.data[inds.flatten(), :]
    ...
    for n in range(1, DiscreteCurve_data.shape[0]-1):
        new_point = (
            DiscreteCurve_data[n-1] + DiscreteCurve_data[n+1] + DiscreteCurve_data[n]) / 3
    ...
    # Number of points in the discrete shortest path
    N_points = DiscreteCurve.shape[0]
    # The time steps to construct the spline
    ...
    curve_spline = CubicSpline(t, DiscreteCurve)
    dcurve_spline = curve_spline.derivative()  # The derivative of the curve
    def curve(t): return evaluate_spline_solution(
        curve_spline, dcurve_spline, t)

    # Return the solution
    solution = {'curve': curve, 'solver': solver.name,
                'points': DiscreteCurve[1:-1, :], 'time_stamps': t[1:-1]}
    curve_length_eval = curve_length(
        manifold, curve, tol=solver.tol, limit=solver.limit)
...
def expmap(manifold, x, v):
    ...
    def ode_fun(t, c_dc): return second2first_order(
        manifold, c_dc).flatten()  # The solver needs this shape (D,)
    if np.linalg.norm(v) > 1e-5:
        curve, failed = solve_expmap(manifold, x, v, ode_fun)
    else:
        def curve(t): return (x.reshape(D, 1).repeat(np.size(t), axis=1),
...
def solve_expmap(manifold, x, v, ode_fun):
    ...
    # 2D x 1 -> (2D, ), the solver needs this shape
    init = np.concatenate((x, v), axis=0).flatten()
    failed = False

    # Solve the IVP problem
    # First solution of the IVP problem
    solution = solve_ivp(ode_fun, [0, 1], init, dense_output=True)
    def curve(t): return evaluate_solution(
        solution, t)  # with length(c(t)) != ||v||_c
    # the length the geodesic should have
    solution_length = curve_length(manifold, curve, 0, 1)
...
class SolverGraph:
    ...
    def __init__(self, manifold, data, kNN_num, tol=1e-5, limit=50):
        ...
        self.data = data
        # The first point for the training data is always the training data point.
        self.kNN_num = kNN_num + 1
        self.kNN_graph = NearestNeighbors(
            n_neighbors=kNN_num + 1, algorithm='ball_tree').fit(data)  # Find the nearest neighbors
        self.tol = tol
        self.limit = limit
        N_data = data.shape[0]
        ...
        for ni in range(N_data):  # For all the data
            ...
            # For each Euclidean kNN connect with the Riemannian distance
            for nj in range(kNN_num):
                ind_j = kNN_inds[nj]  # kNN index
                p_j = data[ind_j, :].reshape(-1, 1)  # The kNN point
                def temp_curve(t): return evaluate_failed_solution(p_i, p_j, t)
                # Note: Shortest path on graph prefers "low-weight" connections
                Weight_matrix[ni, ind_j] = curve_length(
                    manifold, temp_curve, tol=tol, limit=limit)

            if ni % 100 == 0:
                print(
                    "[Initialize Graph] [Processed point: {}/{}]".format(ni, N_data))
        ...
        self.dist_matrix, self.predecessors = \
            shortest_path(csgraph=self.New_Graph, directed=False,
...
class RicciGraph:
    def __init__(self, manifold, data, kNN_num, tol=1e-5, limit=50, method='Forman', verbose='INFO'):
        self.manifold = manifold
        self.data = data
        N_data = data.shape[0]
        ...
        self.kNN_num = kNN_num + 1
        self.kNN_graph = NearestNeighbors(
            n_neighbors=self.kNN_num + 1, algorithm='ball_tree').fit(data)
        distances, indices = self.kNN_graph.kneighbors(data)
        self.tol = tol
        self.limit = limit
        ...
        # Make the weight matrix symmetric
        Weight_matrix = 0.5 * (Weight_matrix + Weight_matrix.T)
        self.w1 = Weight_matrix
        # Create the graph
        G = nx.Graph(Weight_matrix)

        if method == 'Ollivier':
            ricci_graph = OllivierRicci(G, alpha=0.5, verbose="ERROR")
        elif method == "Forman":
            ricci_graph = FormanRicci(G, verbose="ERROR", method="1d")
        else:
            raise ValueError(
                "Method for computing Ricci curvature must be either Ollivier or Forman")
        self.G_ricci = ricci_graph.compute_ricci_curvature()

        ricci_matrix = neg_sigmoid(
            nx.to_numpy_array(self.G_ricci, weight='ricciCurvature'))
        dist_matrix = nx.to_numpy_array(self.G_ricci, weight='weight')

        Weight_matrix = ricci_matrix*dist_matrix
        self.New_Graph = csr_matrix(
            Weight_matrix, shape=(len(data), len(data)))
        self.dist_matrix, self.predecessors = \
            shortest_path(csgraph=self.New_Graph, directed=False,
                          return_predecessors=True)
        self.name = 'ricci'
...
class SE_Kernel:
    ...
    def kddxdy(self, x, y):
        Kddxdy = (-3.0 * self.delta(x, y) / self.ell2 +
                  self.delta(x, y) ** 3) * self.kxy(x, y)
        return Kddxdy
...
def neg_sigmoid(x, lambda_=0.1, spread=1):
    return spread - (spread / (1.0 + np.exp(-lambda_*x)))
